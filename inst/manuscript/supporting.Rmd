  ...
  sim_table <- sim_table %>%
    ...
    mutate(
     Approach = case_when(
       ...
       Approach == "Design IRS" ~ "GRTS-MB",
       Approach == "Model GRTS" ~ "SRS-DB",
...
sim_table <- sim_table[c(3, 4, 1, 2), ]
sim_table_out <- xtable(
  sim_table,
  digits = 4,
  caption = paste0("Sampling-analysis combination (Approach), population layout (Layout), response type (Response), proportion of dependent random error (DRE\\%), sample size (n), mean bias (MB), root-mean-squared error (rMS(P)E), and 95\\% interval coverage (Coverage) in simulation scenario ", table_value, "."),
   type = "latex",
...
data_table <- data_table[c(3, 4, 1, 2), ]
data_table_out <- xtable(
  data_table,
  digits = 4,
  caption = paste0("Sampling-analysis combination (Approach), population layout (Layout), response type (Response), proportion of dependent random error (DRE\\%), sample size (n), mean bias (MB), root-mean-squared error (rMS(P)E), and 95\\% interval coverage (Coverage) in a data application scenario ", table_value, "."),
   type = "latex",
  latex.environments = "center",
  align = c("lllrrrr")
)
