...
\section{Simulation Study}\label{sec:numstudy}}
...
placed on a fixed, equall spaced grid inside the unit square. The two
...
the proportion of tiems the true mean is contained in its 95\% interval.
...
layout with ``IRS-Design'' as the baseline. The relative rMS(P)E is
...
correlation. When there is spatial correlation (Figure \ref{fig:figeff},
...
the four sampling-analysis combinations in the random location layout.
...
coverage in all 36 simulation scenarios are provided in the
...
various aspects of lake health and water quality for lakes in the
contiguous United States. Specifically, we will analyze mercury
concentration in lakes. Although we know the true mean mercury
concentration values for the 986 lakes from the 2012 NLA, we will
explore whether or not we obtain an adequately precise estimate for the
realized mean mercury concentration if we sample only 100 of the 986
lakes.
...
the empirical semivariogram can be visualized alongside a fit using the
model-based approaches. The empirical semivariogram quantifies the
...
shows the empirical semivariogram quantifying the average semivariance
for several distances. Overlain onto Figure \ref{fig:figsv} is the
estimated semivariance obtained using the covariance parmaeters from the
REML fit of GRTS-Model. Figure \ref{fig:figsv} provides evidence that
there is strong correlation in mercury concentration among the sites.
