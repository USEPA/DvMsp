...
Corvallis, Oregon, 97331}
  ...
  \item
    ...
    We compare the approaches in a finite population spatial context.
  \item
...
three sample sizes). For each sample from each scenario, parameters were
estimated using the same approaches as described for simulated data. We
also used the same evaluation metrics as for the simulated data: mean
bias, RMSE, and interval coverage. Mean bias is taken as the average
deviation between each sample's estimated (or predicted) mean
(\(\hat{\mu}_i\)) and the population mean (\(\mu\)) (of ZMMI or Hg ppb):
\(\frac{1}{n}\sum_{i = 1}^{2000} (\hat{\mu}_i - \mu)\), where \(i\)
indexes simulation trials. Root-mean-squared error is taken as the
square root of the average squared deviation between each sample's
estimated (or predicted) mean and its population mean:
