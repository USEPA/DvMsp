...
define sampling design, define parameters, define model-based, sample, population, super-population?, etc.

introduction of spatial data / definition of spatial and maybe general background?
...
brus2020statistical: most similar to what this paper will be, directly comparing the two approaches. This has a lot of other potentially useful references to go back to. (published in 2021 so likely doesn't have many that have cited it yet).

de1990model: early comparison of design v model in spatial context

Several comparisons between modegitl-based and design-based for spatial data have been considered, but they tend to compare spatial models to IRS (Independent Random Sampling) or SIRS (stratified IRS). No comparison has been made between spatial models and spatially balanced design-based approaches.

* hansen1983evaluation - general overview (in non spatial context), talk about population vs superpopulation give a set of guiding principles in the discussion

* brus1997random - design and model based explanation and simulation study for spatial means, make a decision tree for choosing among approaches

* verhoef2002sampling - strictly defines classical sampling, compares FPBK to classical, BK to classical

* wang2012spatial - sampling and inference steps, thorough overview in terms of spatial context, lots of references
...
Several comparisons between the two paradigms have been made in many different contexts. @sterba2009alternative give psych context, old history fisher / Neyman

Many new methods have been constructed that combine aspects of both design-based and model-based approaches. Our goal is not to expand upon these methods nor is it to develop a novel method that combines the two approaches.  

Design-Based and Model-Based Comparisons 

: psychological context comparison, has a lot of the older background (e.g. Fisher, Neyman, etc.)

Cooper2006sampling: ecological context, also has a nice very general background comparing two approaches, introduces a "model-assisted" variance estimator.
