...
<!-- a sample is spatially balanced -->
...
Finite Population Block Kriging (FPBK) is an alternative to sampling-based methods [@verhoef2008spatial]. FPBK expands the geostatistical kriging framework to the finite population setting. Instead of basing inference off of a specific sampling design, we assume the data are generated by a spatial process with parameters that can be estimated using the framework of a model. 

<!-- don't like this: the framework of a model.  -->

@verhoef2008spatial gives details on the theory of FPBK, but some of the basic principles are summarized below. For a response variable $\mathbf{z}$ that can be measured on a finite number of $N$ sites, we want to predict some linear function of the response variable, $\tau(\mathbf{z}) = \mathbf{b}^\prime \mathbf{z}$, where $\mathbf{b}$ is a vector of weights. For example, if we want to predict the population total across all sites, then we would use a vector of 1's for the weights. 
...
<!-- do we want the general model or do we want a constant mean model with a note that this can be expanded to include covariates? -->
...
<!-- need to add stationarity to this definition -->

<!-- need to define indicator variable definition -->
...
<!-- a little weird to talk about inference being based on distributional assumptions and then throw in this sentence about not assuming a distribution for z. -->
...
<!-- spsurvey is talked about in GRTS. so either move that down here or move this to FPBK. -->
