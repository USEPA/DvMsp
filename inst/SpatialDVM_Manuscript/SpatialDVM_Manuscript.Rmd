...
# mike chunk
# Mike: see if this works. If so, delete one of the duplicates
...
# combo_data <- purrr::map_df(files,
#                             ~read_csv(.x) %>% mutate(filename = .x)) %>%
#   separate(filename, into = c("junk", "junk2", "error_dep", "dist", "n", "location"),
#            sep = "_") %>%
#   select(-junk, -junk2)
...
* Pros of Model-Based (items we are not exploring): covariate inference, more efficient small-area estimation, model selection?
