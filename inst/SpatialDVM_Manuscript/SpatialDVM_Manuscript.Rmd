...
row.names(parm_tab) <- c("Sample Size (n)", "Site Locations", 
                         "Partial Sill Variance Ratio", "Response Type")
...
```{r, figlog, out.width = "100%", fig.cap = "Lognormal distribution with a mean of 0 on the log scale and variance of 2 on the log scale. The distribution exhibits moderate to strong right-skewness."}
mu <- 0; var <- 2
...
ggplot(data = combo_data, aes(x = factor(n), y = rel_efficiency, colour = approach)) +
...
change partial sill variance to proportion of partial sill
