...
Though the design-based and model-based approaches apply to statistical inference in a broad sense, we focus on comparing these approaches for spatial data. 

<!-- ????????? -->
We define spatial data as [blah blah blah, any data collected at specific locations blah blah blah blah]. Examples include [do we want to give a list of general examples?]. 
<!-- ???????????? -->

@de1990model give an early comparison of these approaches, quashing the belief that design-based approaches could not be used for spatially correlated data. Thereafter, several comparisons between design-based and model-based for spatial data have been considered, but they tend to compare design-based approaches that ignore spatial location in sampling to model-based approaches [@brus1997random; @verhoef2002sampling; @verhoef2008spatial]. More recent overviews include @brus2020statistical and @wang2012spatial, but no numerical comparison has been made between design-based approaches that incorporate spatial location in sampling and model-based approaches.
