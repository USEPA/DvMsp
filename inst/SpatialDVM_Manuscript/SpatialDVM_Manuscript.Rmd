...
There are two approaches to statistical inference: design-based and model-based. Both approaches aim to study a population. Because data generally cannot be sampled at all units in the population (known as population units), the population units must be sampled. In the design-based approach, the samples are often used to estimate parameters of a population. Properties of these estimators are derived through random selection of the samples. In the model-based approach, the samples are often used to predict random variables of a population. Properties of these predictions are derived through a set of distributional assumptions that specify the population's structure. Randomness is crucial to both approaches but for different reasons; the design-based approach uses randomness when selecting a sample of population units, and the model-based approach uses randomness when specifying distributional assumptions. @sterba2009alternative provides historical context for each approach, and @hansen1983evaluation discusses the advantages of each approach [@hansen1983evaluation].

<!-- Shift focus to spatial data -->

Though the design-based and model-based approaches apply to statistical inference in a broad sense, we focus comparing these approaches for spatial data. @de1990model gave an early comparison of these approaches, quashing the belief that design-based approaches could not be used for spatially correlated data. Thereafter, several comparisons between design-based and model-based for spatial data have been considered, but they tend to compare design-based approaches that ignore spatial location in sampling to model-based approaches [@brus1997random, @verhoef2002sampling, @verhoef2008spatial]. No comparison has been made design-based approaches that incorporate spatial location in sampling and model-based approaches.
...
In Section \ref{sec:background}, we compare sampling and estimation procedures between the design-based approach and the model-based approach. In Section \ref{sec:numstudy}, we use simulated and real data to compare the properties of parameter estimates from both aproaches. And in Section \ref{sec:discussion}, we end with a discussion and provide directions for future research. 
