...
sim_output <- parLapply(
  ...
  gridded = FALSE, # grid (TRUE) or random (FALSE) both in [0, 1] x [0,1]
...
if (write_out) {
  library(readr)
  # write_csv(all_output, "inst/all_output/sim_deperr_lognormalbig_n150_randloc.csv")
  write_csv(summ_output, "inst/output/sim_deperr_lognormalbig_n150_randloc.csv")
}
