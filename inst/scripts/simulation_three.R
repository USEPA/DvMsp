...
sim_output <- parLapply(cluster, seed, safely(sim_trial), N = 30^2, n = 150,
                         psill = 0.9, nugget = 0.1, erange = sqrt(4),
                         gridded = FALSE, resptype = "lognormal")
...
if (write_out) {
  library(readr)
  write_csv(all_output, "inst/output/sim_three/all_output.csv")
  write_csv(summ_output, "inst/output/sim_three/summ_output.csv")
}
