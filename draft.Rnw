...
\begin{enumerate}
  ...
  \item Provide reliable software and code: Current literature presents the ideas but do not provide adequate access to user-friendly software amd cpde. We would clearly explain how to use `spsurvey` to design and analyze spatial probability samples and `sptotal` for FBPK. We would make a GitHub repository for the paper walking through code used throughout the manuscript.
