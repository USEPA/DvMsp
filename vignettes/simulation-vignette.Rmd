...
```{r, message = FALSE}
out1 <- sim_trial(N = 900, n = 100, gridded = FALSE,
          cortype = "Exponential", cortype_est = "Exponential",
          psill = 1.8, range = sqrt(4) / 3, nugget = 0.2,
...
```{r}
n_trials <- 5
out5 <- purrr::rerun(n_trials, sim_trial(N = 900, n = 100, gridded = FALSE,
                                         cortype = "Exponential",
                                         cortype_est = "Exponential",
                                         psill = 1.8, range = sqrt(4) / 3,
                                         nugget = 0.2,
                                         resptype = "normal", mu = 0)) %>%
  dplyr::bind_rows()
```
...
out5 %>% dplyr::mutate(resid = true_mean - estimate,
  ...
  dplyr::summarise(mpbias = mean(resid),
