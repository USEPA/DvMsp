...
#' }
#' @examples
#' sim_trial_both(seed = sample.int(1e7, size = 1), N = 100, n = 50,
#' cortype = "Exponential", psill = 1, erange = 1,
...
## library(spsurvey)
## library(dplyr)
sim_trial_both <- function(seed = sample.int(1e7, size = 1),
                      N = 100, n = 50,
  ...
  # simulate some data
  data_both <- sim_pop_both(N, cortype, psill, erange, nugget, resptype)

  analyze_both <- function(data = data_both) {
  ...
  output_list <- purrr::map(data_both, analyze_both)
  output_list[[1]]$location <- "grid"
  output_list[[2]]$location <- "rand"
